...
# create odyssey theme dir and symlink
dest="$vault/.obsidian/themes/Odyssey"
mkdir -p "$dest"
ln -sf "$src" "$dest/theme.css"

# create manifest if missing
[[ -f "$dest/manifest.json" ]] || echo '{"name":"Odyssey","version":"1.0.0","minAppVersion":"0.16.0"}' > "$dest/manifest.json"
exit 0
